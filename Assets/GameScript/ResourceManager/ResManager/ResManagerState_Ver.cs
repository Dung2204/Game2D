...
public class ResManagerState_Ver : ccMachineStateBase
{
    ...
    public override void f_Execute()
    {
        ...
        if (w.error != null)
        {
MessageBox.DEBUG("Line error");
            LoadFail();
        }
        else if (w.text.Length < 4)
        {
MessageBox.DEBUG("Line error 2");
            LoadFail();
        }
        else
        {
    ...
    private void LoadVerSuc(string strVerData)
    {
        ...
        if (aData.Length >= 5)
        {
            ...
            int iServerVer = ccMath.atoi(aData[4]);
MessageBox.DEBUG("Current Version " + GloData.glo_iVer + "-" + iServerVer);
            if (GloData.glo_iVer != iServerVer)
            {
                GloData.glo_iHaveNewApk = 1;
MessageBox.DEBUG("Need to update version " + iServerVer);
            }
            else
            {

                GloData.glo_iHaveNewApk = 0;
            }
            glo_Main.GetInstance().m_UIMessagePool.f_Broadcast(MessageDef.START_LOG);

            f_SetComplete((int)EM_ResManagerStatic.LoadSC, _bSaveCatchBuf);
        }
        else
        {
MessageBox.DEBUG("Download version failed");
        }
    }
    ...
    { 
        ...
        {
            bFileEro = true;
MessageBox.DEBUG("The command file did not perform the required update");
        }

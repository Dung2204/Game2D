...
public class EquipManage : UIFramwork
{
    /// <summary>
    /// 装备信息
    /// </summary>
    ...
    protected override void UI_OPEN(object e)
    {
        ...
        RefinePillIndex = 0;
        Tempid = UITool.f_GetGoodsForEffect(EM_GoodsEffect.EquipRefineExp);
        Initialize();
        f_Message();
    ...
    protected override void UI_CLOSE(object e)
    {
        ...
        if (tmpEquip.oType == EquipBox.OpenType.Master)
        {
            ccUIHoldPool.GetInstance().f_UnHold();
        }
        else if (tmpEquip.oType == EquipBox.OpenType.SelectAward)
        {
            ccUIHoldPool.GetInstance().f_UnHold();
        }
    }
    ...
    private void CheckRedPoint()
    {
        ...
        Data_Pool.m_TeamPool.f_CheckTeamEquipRedPoint(_EquipPool, (EM_EquipPart)_EquipPool.m_EquipDT.iSite, ref isCanLvUp, ref isCanEquip);
    ...
    {
        UI_ChangeUI((EquipBox.BoxTye)obj1);
    }
    private void UI_ShowBtn()
    {
        f_GetObject("Btn_Left").SetActive(tmpEquip.oType == EquipBox.OpenType.Battle);
        f_GetObject("Btn_Right").SetActive(tmpEquip.oType == EquipBox.OpenType.Battle);
        f_GetObject("Chage").SetActive(tmpEquip.oType == EquipBox.OpenType.Battle);
        f_GetObject("Btn_GetWay").SetActive(tmpEquip.oType == EquipBox.OpenType.SelectAward);
        f_GetObject("Btn_Intensify").SetActive(tmpEquip.oType != EquipBox.OpenType.SelectAward);

        GameParamDT gameParam = glo_Main.GetInstance().m_SC_Pool.m_GameParamSC.f_GetSC((int)EM_NeedLevel.EquipRefine) as GameParamDT;
        ...
        f_GetObject("Btn_Refine").SetActive(myLv >= showLv && tmpEquip.oType != EquipBox.OpenType.SelectAward);


        f_GetObject("Btn_UpStars").SetActive(_EquipPool.m_EquipDT.iColour > (int)EM_Important.Purple && UITool.f_GetIsOpensystem(EM_NeedLevel.EquipUpStar) && tmpEquip.oType != EquipBox.OpenType.SelectAward);
        f_GetObject("Down").SetActive(tmpEquip.oType == EquipBox.OpenType.Battle);
        //f_GetObject("Btn_UpStars").SetActive(true);
    }
    private void UI_ChangeUI(EquipBox.BoxTye tBox)
    {
        if (tBox == EquipBox.BoxTye.Refine)
        {
            //判断是否达到精炼等级
        ...
        f_GetObject("Introduce").SetActive(tBox == EquipBox.BoxTye.Intro);
        f_GetObject("Intensify").SetActive(tBox == EquipBox.BoxTye.Inten);
        f_GetObject("Refine").SetActive(tBox == EquipBox.BoxTye.Refine);
        f_GetObject("UpStars").SetActive(tBox == EquipBox.BoxTye.UpStar);
        f_GetObject("GetWay").SetActive(tBox == EquipBox.BoxTye.GetWay);

        UI_ChangeBtn(f_GetObject("Btn_Introduce"), tBox == EquipBox.BoxTye.Intro);
        UI_ChangeBtn(f_GetObject("Btn_Intensify"), tBox == EquipBox.BoxTye.Inten);
        UI_ChangeBtn(f_GetObject("Btn_Refine"), tBox == EquipBox.BoxTye.Refine);
        UI_ChangeBtn(f_GetObject("Btn_UpStars"), tBox == EquipBox.BoxTye.UpStar);
        UI_ChangeBtn(f_GetObject("Btn_GetWay"), tBox == EquipBox.BoxTye.GetWay);
        switch (tBox)
        {
            case EquipBox.BoxTye.Intro:
                UpdateIntroduce();
                break;
            case EquipBox.BoxTye.Inten:
                UpdateIntensify();
                break;
    ...
    void UI_ChageEquip(GameObject go, object obj1, object obj2)
    {
        ...
        for (; time < 4; tpos += (int)obj1)
        {
            ...
            {
                tmpEquip.tEquipPoolDT = tmpEquip.m_TeamPool.m_aEquipPoolDT[tpos - 1];
                _EquipPool = tmpEquip.tEquipPoolDT;
                break;
            }
            time++;
		...
		f_GetObject("Btn_UpStars").SetActive(_EquipPool.m_EquipDT.iColour > (int)EM_Important.Purple && UITool.f_GetIsOpensystem(EM_NeedLevel.EquipUpStar) && tmpEquip.oType != EquipBox.OpenType.SelectAward);
        if(!(_EquipPool.m_EquipDT.iColour > (int)EM_Important.Purple && UITool.f_GetIsOpensystem(EM_NeedLevel.EquipUpStar) && tmpEquip.oType != EquipBox.OpenType.SelectAward))
		{
			UI_ChangeUI(EquipBox.BoxTye.Intro);
		}
		UpdateMain();
    ...
    void UI_CloseIntroduce(GameObject go, object obj1, object obj2)
    {
        ...
        ccUIManage.GetInstance().f_SendMsg(UINameConst.EquipManage, UIMessageDef.UI_CLOSE);
    }
    ...
    void UI_DownEquip(GameObject go, object obj1, object obj2)
    {
        ...
        Data_Pool.m_TeamPool.f_ChangeTeamEquip((long)tmpEquip.m_TeamPool.m_eFormationPos, 0, (byte)tmpEquip.tEquipPoolDT.m_EquipDT.iSite, OnChangeEquipCallback);
    }
    ...
    private void Btn_OneKeyRefine(GameObject go, object obj1, object obj2)
    {
        ...
        int OpenLv = Data_Pool.m_EquipPool.m_OneKeyRefineLv;
           ...
           VipLvNow != 0)
        {
            ...
            OneKeyRefineParam ttttt = new OneKeyRefineParam();
            ...
            ccUIManage.GetInstance().f_SendMsg(UINameConst.OneKeyEquipRefine, UIMessageDef.UI_OPEN, ttttt);
            return;
        }
    ...
    void UpdateMainStar()
    {
        ...
        //return;
        f_GetObject("StarGrid5").SetActive(UITool.f_GetIsOpensystem(EM_NeedLevel.EquipUpStar) && _EquipPool.m_EquipDT.iColour == (int)EM_Important.Red);
        f_GetObject("StarGrid3").SetActive(UITool.f_GetIsOpensystem(EM_NeedLevel.EquipUpStar) && _EquipPool.m_EquipDT.iColour == (int)EM_Important.Oragen);
    ...
    void UpdateIntroduce()
    {
        ...
        TeamPoolDT tTeamPool = UITool.f_GetTeamPoolDTForEquip(_EquipPool);
        if (tTeamPool != null)
            Data_Pool.m_TeamPool.f_UpdateSetEquip(tTeamPool);
        ////////信息界面  属性简介/////////////
        ...
        ProTab_Pro.text = UITool.f_GetEquipPro(_EquipPool).ToString();
        ...
        if (_EquipPool.m_lvRefine > 0)
        {
            ...
            UITool.f_UpdateAddPro(_EquipPool.m_EquipDT.iRefinProId1, RefineTab_Pro1, UITool.f_GetEquipRefinePro(_EquipPool)[0]);
            RefineTab_Pro2Name.text = UITool.f_GetProName((EM_RoleProperty)_EquipPool.m_EquipDT.iRefinProId2) + ":";
            UITool.f_UpdateAddPro(_EquipPool.m_EquipDT.iRefinProId2, RefineTab_Pro2, UITool.f_GetEquipRefinePro(_EquipPool)[1]);
            //RefineTab_Master.text = UITool.f_GetEquipRefineMaster(_EquipPool);
        }
        RefineTab.gameObject.SetActive(_EquipPool.m_lvRefine > 0);
        ////////////////////////套装简介//////////////////////////////
        if (_EquipPool.m_lvRefine > 0)
            SetEuqipTab.transform.localPosition = new Vector3(0, RefineTab.localPosition.y + RefineTab_Master.transform.localPosition.y - 0 - _TabSpace, 0);
        else
            //SetEuqipTab.transform.localPosition = new Vector3(0, RefineTab.localPosition.y - 0 - _TabSpace, 0);
            SetEuqipTab.transform.localPosition = new Vector3(0, ProTab_Master1.transform.localPosition.y - 0 - _TabSpace, 0);
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            if (tTeamPool != null)
                if (_EquipPool.m_SetEquip != null)
                    if (_EquipPool.m_SetEquip.m_aSetIsOk[i])
                        CreareSetEquipEffect(SetEquipTab_Body.transform.GetChild(i).GetChild(4), tEquipid[i]);
        }
        ...
        //////////////////信息界面   说明简介/////////////////////////
        if (tShowSetEquip)
            ParticularsTab.transform.localPosition = new Vector3(0, RefineTab.localPosition.y + RefineTab_Master.transform.localPosition.y - RefineTab_Master.height - _TabSpace, 0);
        else
            ParticularsTab.transform.localPosition = new Vector3(0, SetEuqipTab.localPosition.y - 244 - _TabSpace - SetEquipPro.height);
    ...
    {
        ///////////////升级界面/////////////////////
        Intensify_StarLevelLeft.text = Intensify_StartLevel.text = string.Format("{0}/{1}", _EquipPool.m_lvIntensify, UITool.f_GetEquipIntenMax());
        if (_EquipPool.m_lvIntensify + 1 > UITool.f_GetEquipIntenMax())
            Intensify_LastLevel.text = string.Format("{0}/{1}[-]", _EquipPool.m_lvIntensify + 1, UITool.f_GetEquipIntenMax());
        else
            Intensify_LastLevel.text = string.Format("{0}/{1}[-]", _EquipPool.m_lvIntensify + 1, UITool.f_GetEquipIntenMax());
        if (Data_Pool.m_UserData.f_GetProperty((int)EM_UserAttr.eUserAttr_Money) < UITool.f_GetEquipIntenCon(_EquipPool, 1))
            Intensify_Consume.text = string.Format("[ff0000]{0}", UITool.f_GetEquipIntenCon(_EquipPool, 1).ToString());
        else
            Intensify_Consume.text = UITool.f_GetEquipIntenCon(_EquipPool, 1).ToString();
        Intensify_ProRight.text = Intensify_ProName.text = UITool.f_GetProName((EM_RoleProperty)_EquipPool.m_EquipDT.iIntenProId) + ":";
        Intensify_Pro.text = UITool.f_GetEquipPro(_EquipPool).ToString();
    ...
    void UpdateRefine()
    {
        ...
        UITool.f_UpdateAddPro(_EquipPool.m_EquipDT.iRefinProId1, Refine_Pro1, UITool.f_GetEquipRefinePro(_EquipPool)[0]);
        UITool.f_UpdateAddPro(_EquipPool.m_EquipDT.iRefinProId2, Refine_Pro2, UITool.f_GetEquipRefinePro(_EquipPool)[1]);
        ...
        if (_EquipPool.m_lvRefine != 50)
        {
            Refine_ShowExp.value = (float)_EquipPool.m_iexpRefine / UITool.f_GetEquipRefineExp(_EquipPool);
            Refine_Exp.text = string.Format("{0}/{1}", _EquipPool.m_iexpRefine, UITool.f_GetEquipRefineExp(_EquipPool));
        }
        else
        {
            Refine_ShowExp.value = 1;
            Refine_Exp.text = "Tối đa";
        }
        ...
        for (int i = 0; i < tgo.Length; i++)
            UITool.f_SetIconSprite(tgo[i].GetComponent<UI2DSprite>(), EM_ResourceType.Good, 101 + i);
        if (_EquipPool.m_lvRefine == 50)
            return;
        /////////////////////////精炼界面///////////////////////////////
    ...
    void UpdateUpStar(bool ischange = true)
    {
        ...
        EquipUpStarDT tmpUpStar = UITool.f_GetEquipUpStar(_EquipPool);
        ...
        if (tmpBtnUp[2] > 0)
        {
            UpStar_UpOptions.Find("FragmentUp/Fragment/FragmentNum").GetComponent<UILabel>().text = tmpUpStar.iDebrisNum > UITool.f_GetEquipFragmentNum(tmpUpStar.iDebrisId) ? string.Format("[ff0000]{0}[-]", tmpUpStar.iDebrisNum) : tmpUpStar.iDebrisNum.ToString();
        }
        if (ischange)
        {
            for (int i = 0; i < 3; i++)
    ...
    void UpdateUpStarHead()
    {
        ...
        tmpBtnUp[1] = tmpUpStar.iGoldNum2;
        tmpBtnUp[2] = tmpUpStar.iDebrisNum;
        int[] tmpInt = UITool.f_GetEquipStarPro(_EquipPool);
        switch (_EquipPool.m_EquipDT.iColour)
        {
            case (int)EM_Important.Oragen:
                ...
                else
                {
                    ...
                    UpStar_EndPro.text = "Tối đa";
                    UpStar_AddPro.text = "Tối đa";
                    UpStar_StarNeedExp.text = "Tối đa";
                }
                break;
        ...
        UpStar_SucRate.text = UITool.f_GetStarSucRate(_EquipPool);
    ...
    void Equip_UpStars()
    {
        EquipUpStarDT tmpStar = UITool.f_GetEquipUpStar(_EquipPool);
        ...
        switch (_UpStarType)
        {
            case 1:
                ...
                Data_Pool.m_EquipPool.f_EquipUpStar(_EquipPool.iId, EM_EquipUpStarType.GoldUp, Callback);
                break;
            case 3:
                if (tmpStar.iDebrisNum >
                           UITool.f_GetEquipFragmentNum(tmpStar.iDebrisId))
                {
                    UITool.Ui_Trip(CommonTools.f_GetTransLanguage(1740));
                    ccTimeEvent.GetInstance().f_UnRegEvent(Time_SedUpStar);
                    return;
                }
                Data_Pool.m_EquipPool.f_EquipUpStar(_EquipPool.iId, EM_EquipUpStarType.FragmentUp, Callback);
                break;
            case 2:
                ...
                Data_Pool.m_EquipPool.f_EquipUpStar(_EquipPool.iId, EM_EquipUpStarType.MoneyUp, Callback);
                break;
        }
    }
    /// <summary>
    ...
    void ClickRefine(object Obj)
    {
        ...
        UpdateRefine();
        if (_EquipPool.m_iexpRefine >= UITool.f_GetEquipRefineExp(_EquipPool))
        {
            ...
            _EquipPool.m_iexpRefine -= UITool.f_GetEquipRefineExp(_EquipPool);
        ...
        if (_EquipPool.m_iexpRefine + tGoods.m_BaseGoodsDT.iEffectData * RefineTimes >= UITool.f_GetEquipRefineExp(_EquipPool))
        {
            RefineTimes = 1;
        }
    ...
    void SendEquipRefine(int index)
    {
        ...
        Data_Pool.m_EquipPool.f_Refine(_EquipPool.iId, Tempid[index], RefinePillNum[index], Callback);
    ...
    {
        if (_EquipPool.m_lvIntensify >= UITool.f_GetEquipIntenMax())
        {
            UITool.Ui_Trip(CommonTools.f_GetTransLanguage(1752));
            return;
        }

        if (Data_Pool.m_UserData.f_GetProperty((int)EM_UserAttr.eUserAttr_Money) < UITool.f_GetEquipIntenCon(_EquipPool, 1))
        {
            UITool.Ui_Trip(CommonTools.f_GetTransLanguage(1753));
            GetWayPageParam tGetWayParm = new GetWayPageParam(EM_ResourceType.Money, (int)EM_MoneyType.eUserAttr_Money, this);
            ccUIManage.GetInstance().f_SendMsg(UINameConst.GetWayPage, UIMessageDef.UI_OPEN, tGetWayParm);
            return;
        }
        UITool.f_OpenOrCloseWaitTip(true);
        Data_Pool.m_EquipPool.f_LevelUp(_EquipPool.iId, (short)(1 + _EquipPool.m_lvIntensify), InterSuc);
    }
    /// <summary>
    /// 强化五次
    /// </summary>
    ...
    void EquipInten5(GameObject go, object obj1, object obj2)
    {
        ...
        if (Data_Pool.m_UserData.f_GetProperty((int)EM_UserAttr.eUserAttr_Money) < UITool.f_GetEquipIntenCon(_EquipPool, 5))
        {
        ...
        UITool.f_OpenOrCloseWaitTip(true);
        if (_EquipPool.m_lvIntensify + 5 >= UITool.f_GetEquipIntenMax())
            Data_Pool.m_EquipPool.f_LevelUp(_EquipPool.iId, (short)(UITool.f_GetEquipIntenMax()), InterSuc);
        else
            Data_Pool.m_EquipPool.f_LevelUp(_EquipPool.iId, (short)(5 + _EquipPool.m_lvIntensify), InterSuc);
    ...
    void UpStar(object obj)
    {
        int ui = (int)obj;
        Equip_UpStars();
        _ChangeUpStarType(UpStar_UpOptions.GetChild(ui).gameObject, ui);
    }
    #region 图鉴打开
    /// <summary>
    /// 图鉴打开
...
/// 装箱
/// </summary>
public struct EquipBox : Box
{
    public EquipPoolDT tEquipPoolDT;
    public BoxTye tType;
...
/// 装备升星类型
/// </summary>
public enum EM_EquipUpStarType
{
    /// <summary>
    /// 银币升星
    /// </summary>

...
public class OneKeyEquipRefine : UIFramwork
{
    private OneKeyRefineParam _OneKeyRefineParam;
    private EquipPoolDT _EquipPoolDt;
    ...
    {
        base.UI_OPEN(e);
        if(!(e is OneKeyRefineParam) && e == null)
        {
MessageBox.ASSERT("Quick Refinement, input cannot be Null or EquipPoolDt");
        }
        _OneKeyRefineParam = (OneKeyRefineParam)e;
        ...
        {
UITool.Ui_Trip("Không đủ Đá tinh luyện để tinh luyện nhanh");
            GetWayPageParam tGetWayParm = new GetWayPageParam(EM_ResourceType.Good, 103, _OneKeyRefineParam.m_CallUIBase);
...
strRefineNow = "[f1b049]Tinh luyện:[-][fffaec]{0}";
    ...
    {
        ccUIManage.GetInstance().f_SendMsg(UINameConst.OneKeyEquipRefine, UIMessageDef.UI_CLOSE);
    }
    ...
    private void Btn_Suc(GameObject go, object obj1, object obj2)
    {
        ...
        if(isZero)
        {
UITool.Ui_Trip("Hãy chọn mức độ");
            return;
        }

        UITool.f_OpenOrCloseWaitTip(true);
        Data_Pool.m_EquipPool.f_EquipOneKeyRefine(_EquipPoolDt.iId, 4, Tempid, Consome, OneKeyRefineCallback);
    }
    ...
    private void UpdateMainUi()
    {
        ...
        tItem.f_UpdateByInfo((int)EM_ResourceType.Equip, _EquipPoolDt.m_iTempleteId, 1);
...
public class OneKeyRefineParam
{
    public EquipPoolDT m_EquipPoolDT;

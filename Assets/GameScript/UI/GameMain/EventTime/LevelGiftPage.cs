using UnityEngine;
using System.Collections;
using System.Collections.Generic;
using ccU3DEngine;
using System;
using Spine;
using Spine.Unity;
/// <summary>
/// 活动首充界面
/// </summary>
public class LevelGiftPage : UIFramwork
{
    private SocketCallbackDT RequestGetCallback = new SocketCallbackDT();//领取回调
    private bool initUI;
    EventTimeDT eventTimeDT;
    List<NBaseSCDT> LevelGiftDTs;
    private int index_LevelGiftDTs;
    EventTimePoolDT tPoolDataDT;
    private UILabel ActivityEndTime;   //活
    private bool mFirstPage = true;
    private UIScrollView mScrollView;
    private GameObject mAwardItemParent;
    private GameObject mAwardItem;

    private GameObject m_LeftArrow;
    private GameObject m_RightArrow;

    string path1 = "UI/TextureRemove/LevelGift/Char1";
    string path2 = "UI/TextureRemove/LevelGift/Char2";
    string path3 = "UI/TextureRemove/LevelGift/Char3";

    protected override void UI_OPEN(object e)
    {
        base.UI_OPEN(e);
        if (initUI)
        {
            EventTimeDT newEvent = (EventTimeDT)e;

            if (newEvent.iId != eventTimeDT.iId)
            {
                initUI = false;
            }
        }

        eventTimeDT = (EventTimeDT)e;
        Debug.Log(eventTimeDT.iId);
        UpdateContent();

    }

    protected override void f_CustomAwake()
    {
        base.f_CustomAwake();
        InitGUI();
    }
    protected override void InitGUI()
    {
        base.InitGUI();
        m_LeftArrow = f_GetObject("Left");
        m_RightArrow = f_GetObject("Right");

    }

    private void f_OnMomnetEnds()
    {
        Vector3 constraint = mScrollView.panel.CalculateConstrainOffset(mScrollView.bounds.min, mScrollView.bounds.min);
        if (constraint.y <= 0)
        {
            mFirstPage = false;
            Data_Pool.m_RankingPowerAwardPool.f_RankList();
        }
    }

    protected override void f_Create()
    {
        _InitReference();
        base.f_Create();
    }
    private void _InitReference()
    {
        AddGOReference("Panel/Anchor-Center/BlackBg");
        AddGOReference("Panel/Anchor-Center/UI/BtnClose");
        AddGOReference("Panel/Anchor-Center/UI/Item");

        AddGOReference("Panel/Anchor-Center/UI/Type1Panel/DayNum/ActivityEndTime");

        AddGOReference("Panel/Anchor-Center/UI/Type1Panel");
        AddGOReference("Panel/Anchor-Center/UI/Type1Panel/Left");
        AddGOReference("Panel/Anchor-Center/UI/Type1Panel/Right");
        AddGOReference("Panel/Anchor-Center/UI/Type1Panel/PayDesc");
        AddGOReference("Panel/Anchor-Center/UI/Type1Panel/PayDescType");
        AddGOReference("Panel/Anchor-Center/UI/Type1Panel/Char");
    }
    /// <summary>
    /// 加载texture
    /// </summary>
    private void f_LoadTexture()
    {

    }
    protected override void UI_UNHOLD(object e)
    {
        base.UI_UNHOLD(e);
    }
    protected override void f_InitMessage()
    {
        base.f_InitMessage();
        f_RegClickEvent("BlackBg", OnCloseThis);
        f_RegClickEvent("BtnClose", OnCloseThis);
        f_RegClickEvent("Left", OnLeftClick);
        f_RegClickEvent("Right", OnLeftRight);

        //glo_Main.GetInstance().m_UIMessagePool.f_AddListener(UIMessageDef.UI_RANKING_POWER_LIST, OnGetSucCallback, this);

        RequestGetCallback.m_ccCallbackSuc = OnGetSucCallback;
        RequestGetCallback.m_ccCallbackFail = OnGetFailCallback;

    }

    private void OnCloseThis(GameObject go, object obj1, object obj2)
    {
        ccUIManage.GetInstance().f_SendMsg(UINameConst.LevelGiftPage, UIMessageDef.UI_CLOSE);
        f_DestoryView();

    }

    private void BtnPayType1(GameObject go, object obj1, object obj2)
    {
        LevelGiftDT levelGiftDT = (LevelGiftDT)obj1;

        int uCoin = Data_Pool.m_UserData.f_GetProperty((int)EM_UserAttr.eUserAttr_Coin);

        EventTimePoolDT eventTimePoolDT = (EventTimePoolDT)Data_Pool.m_EventTimePool.f_EventTimePoolDT(levelGiftDT.iEventTime, levelGiftDT.iId);
        if (eventTimePoolDT.idata3 >= levelGiftDT.iMaxNum)
        {
UITool.Ui_Trip("Đã đạt giới hạn mua!");
            return;
        }
        if (eventTimePoolDT.idata2 < GameSocket.GetInstance().f_GetServerTime())
        {
UITool.Ui_Trip("Đã hết hạn!");
            return;
        }

        if (uCoin >= levelGiftDT.iPayNum)
        {
            MessageBox.ASSERT("TsuLog: Co the mua " + uCoin + " >= " + levelGiftDT.iPayNum);
            //SocketCallbackDT whitelistCallbackDt = new SocketCallbackDT();
            //whitelistCallbackDt.m_ccCallbackSuc = f_OnRechargeCoin;
            //whitelistCallbackDt.m_ccCallbackFail = f_OnRechargeCoin;
            //Data_Pool.m_RechargePool.RechargeTripleMoney(tripleMoneyDT.iEventTime, tripleMoneyDT.iId, whitelistCallbackDt);


PopupMenuParams tParam = new PopupMenuParams("Xác Nhận", string.Format("Dùng {0} Kim Phiếu \n để mua {1}", levelGiftDT.iPayNum, levelGiftDT.szPayDesc), "Đồng ý", f_ConfirmRecharge, "Hủy bỏ" , null, new int[] { levelGiftDT.iEventTime, levelGiftDT.iId });
            ccUIManage.GetInstance().f_SendMsg(UINameConst.PopupMenuPage, UIMessageDef.UI_OPEN, tParam);
            MessageBox.ASSERT("TsuLog: Co the mua " + uCoin + " >= " + levelGiftDT.iPayNum);
        }
        else
        {
            MessageBox.ASSERT("TsuLog: Khong du kiem phieu de giao dich " + uCoin + " < " + levelGiftDT.iPayNum);
UITool.Ui_Trip("Không đủ Kim Phiếu");
            //glo_Main.GetInstance().m_SDKCmponent.f_Pccaccy(0, "", 0, 0, tCPOrderId, "", 0, "", "");
            glo_Main.GetInstance().m_SDKCmponent.f_ShowSDKPay();
        }

        //----------------------------------
    }

    private void f_ConfirmRecharge(object value)
    {
        int[] param = (int[])value;

        int iEventTime = param[0];
        int orderId = param[1];

        idtriple = orderId;
        SocketCallbackDT whitelistCallbackDt = new SocketCallbackDT();
        whitelistCallbackDt.m_ccCallbackSuc = f_OnRechargeCoin;
        whitelistCallbackDt.m_ccCallbackFail = f_OnRechargeCoin;
        Data_Pool.m_RechargePool.RechargeLevelGift(iEventTime, orderId, whitelistCallbackDt);
    }

    private int idtriple; 
    private void f_OnRechargeCoin(object result)
    {
        if ((int)result == (int)eMsgOperateResult.OR_Succeed)
        {
UITool.Ui_Trip("Mua hàng thành công");
            //LevelGiftDT levelGiftDT = (LevelGiftDT)glo_Main.GetInstance().m_SC_Pool.m_LevelGiftSC.f_GetSC(idtriple);
            //List<AwardPoolDT> tAwardPoolList = new List<AwardPoolDT>();

            //string[] awardSubGroup = levelGiftDT.szAward.Split('#');
            //for (int i = 0; i < awardSubGroup.Length; i++)
            //{
            //    string[] awardItem = awardSubGroup[i].Split(';');
            //    AwardPoolDT taward = new AwardPoolDT();
            //    taward.f_UpdateByInfo((byte)int.Parse(awardItem[0]), (int)int.Parse(awardItem[1]), int.Parse(awardItem[2]));
            //    tAwardPoolList.Add(taward);
            //}

            //if (tAwardPoolList.Count > 0)
            //{
            //    ccUIManage.GetInstance().f_SendMsg(UINameConst.GainAwardShowPage, UIMessageDef.UI_OPEN, new object[] { tAwardPoolList });
            //    tAwardPoolList.Clear();
            //}
            //f_UpdateInfo((LevelGiftDT)LevelGiftDTs[index_LevelGiftDTs], f_GetObject("Type1Panel"));
            UpdateContent();
        }
        else
        {
UITool.Ui_Trip("Mua hàng không thành công");
            MessageBox.ASSERT("Recharge whitelist failed,code:" + (int)result);
        }
    }

    private void f_UpdateInfo(LevelGiftDT levelGiftDT, GameObject Parent)
    {

        Transform Item = f_GetObject("Item").transform;

        EventTimePoolDT eventTimePoolDT = (EventTimePoolDT)Data_Pool.m_EventTimePool.f_EventTimePoolDT(levelGiftDT.iEventTime, levelGiftDT.iId);
        if (eventTimePoolDT.idata4 != 0 && levelGiftDT.iType == 2)
        {
            LevelGiftDT levelGiftDT_temp = (LevelGiftDT)glo_Main.GetInstance().m_SC_Pool.m_LevelGiftSC.f_GetSC(eventTimePoolDT.idata4);
            levelGiftDT.iPayNum = levelGiftDT_temp.iPayNum;
            levelGiftDT._szPayDesc = levelGiftDT_temp._szPayDesc;
            levelGiftDT.szAward = levelGiftDT_temp.szAward;
            levelGiftDT.iPayCount = levelGiftDT_temp.iPayCount;
        }
        List<ResourceCommonDT> listCommonDT = CommonTools.f_GetListCommonDT(levelGiftDT.szAward);

        UILabel Name = Parent.transform.Find("Name").GetComponent<UILabel>();
        Name.text = levelGiftDT.szPayDesc;

        UILabel Num = Parent.transform.Find("PayNum").GetComponent<UILabel>();

Num.text = string.Format("Giới hạn : {0}/{1} lần", levelGiftDT.iMaxNum - eventTimePoolDT.idata3, levelGiftDT.iMaxNum);

        f_RegClickEvent(Parent.transform.Find("BtnPayType").gameObject, BtnPayType1, levelGiftDT);
        UILabel Namebtn = Parent.transform.Find("BtnPayType/Label").GetComponent<UILabel>();
Namebtn.text = string.Format("{0} Kim Phiếu", levelGiftDT.iPayNum);

        ActivityEndTime = f_GetObject("ActivityEndTime").GetComponent<UILabel>();

        int second_chenhlech = Data_Pool.m_UserData.m_CreateTime - Data_Pool.m_SevenActivityTaskPool.OpenSeverTime;
        int second_du2 = second_chenhlech % 86400;

        iTime1 = eventTimePoolDT.idata2 - GameSocket.GetInstance().f_GetServerTime();

        tDate1 = new System.DateTime(1970, 1, 1, 0, 0, 0, System.DateTimeKind.Unspecified).AddSeconds(iTime1);
        if (iTime1 > 0)
        {
            ActivityEndTime.text = string.Format(CommonTools.f_GetTransLanguage(2112), tDate1.Day - 1, tDate1.Hour, tDate1.Minute) + tDate1.Second + CommonTools.f_GetTransLanguage(2114);
        }
        else
        {
            ActivityEndTime.text = string.Format(CommonTools.f_GetTransLanguage(2115));
        }

        UILabel PayDesc = f_GetObject("PayDesc").GetComponent<UILabel>();
        if(levelGiftDT.iType == 1)
        {
PayDesc.text = string.Format("Cấp {0}, nhận 1 phần quà đặc biệt", levelGiftDT.iParam1);
        }
        else
        {
            PayDesc.text = string.Format(levelGiftDT.szPayDesc);
        }


        UILabel PayDescType = f_GetObject("PayDescType").GetComponent<UILabel>();
        PayDescType.text = string.Format("{0}", levelGiftDT.szPayDesc);
        UITexture mTextureBg = f_GetObject("Char").GetComponent<UITexture>();
if (levelGiftDT.szPayDesc == "Return 800%")
        {
            mTextureBg.mainTexture = glo_Main.GetInstance().m_ResourceManager.f_CreateResourceTexture2D(path1);
        }
else if (levelGiftDT.szPayDesc == "Return 900%")
        {
            mTextureBg.mainTexture = glo_Main.GetInstance().m_ResourceManager.f_CreateResourceTexture2D(path2);
        }
        else
        {
            mTextureBg.mainTexture = glo_Main.GetInstance().m_ResourceManager.f_CreateResourceTexture2D(path3);
        }
        mTextureBg.MakePixelPerfect();


        GridUtil.f_SetGridView<ResourceCommonDT>(Parent.transform.Find("Grid").gameObject, Item.gameObject, listCommonDT, UpdateItem);
        f_UpdateArrow();
    }

    private void f_UpdateArrow()
    {
        int count = LevelGiftDTs.Count;
        m_LeftArrow.SetActive(index_LevelGiftDTs > 0);
        m_RightArrow.SetActive(index_LevelGiftDTs < count - 1);
    }

    private void OnLeftClick(GameObject go, object obj1, object obj2)
    {
        index_LevelGiftDTs--;
        if(index_LevelGiftDTs <= 0)
        {
            index_LevelGiftDTs = 0;
        }
        f_UpdateInfo((LevelGiftDT)LevelGiftDTs[index_LevelGiftDTs], f_GetObject("Type1Panel"));

    }

    private void OnLeftRight(GameObject go, object obj1, object obj2)
    {
        index_LevelGiftDTs++;
        if (index_LevelGiftDTs >= LevelGiftDTs.Count - 1)
        {
            index_LevelGiftDTs = LevelGiftDTs.Count - 1;
        }
        f_UpdateInfo((LevelGiftDT)LevelGiftDTs[index_LevelGiftDTs], f_GetObject("Type1Panel"));
    }

    private void f_OnAwardItemClick(Transform tf, BasePoolDT<long> dt)
    { 
    
    }
    int iTime1;
    DateTime tDate1;
    private void UpdateContent()
    {
        // event này được sv trả về pool id
        // check config xem id nào đang acctive thì hiện
        List<NBaseSCDT>  temp_LevelGiftDTs = glo_Main.GetInstance().m_SC_Pool.m_LevelGiftSC.f_GetSCByEventTimeId(eventTimeDT.iId);
        if(temp_LevelGiftDTs.Count <= 0)
        {
            ccUIManage.GetInstance().f_SendMsg(UINameConst.LevelGiftPage, UIMessageDef.UI_CLOSE);
Debug.Log("don't set gift id for this event time");
            return;
        }
        LevelGiftDTs = new List<NBaseSCDT>();
        for (int i = 0; i < temp_LevelGiftDTs.Count; i++)
        {
            LevelGiftDT levelGiftDT = (LevelGiftDT)temp_LevelGiftDTs[i];
            EventTimePoolDT eventTimePoolDT = (EventTimePoolDT)Data_Pool.m_EventTimePool.f_EventTimePoolDT(levelGiftDT.iEventTime, levelGiftDT.iId);
            bool isOpen4 = GameSocket.GetInstance().f_GetServerTime() <= eventTimePoolDT.idata2 && eventTimePoolDT.idata3 < levelGiftDT.iMaxNum;
            if (isOpen4)
            {
                LevelGiftDTs.Add(levelGiftDT);
            }
        }
        //
        if (LevelGiftDTs.Count == 0) {
            OnCloseThis(null, null, null);
            return;
        }

        //cập nhật ui thằng đầu tiên
        index_LevelGiftDTs = 0;
        if(StaticValue.OpenPopupLevelGiftId > 0)
        {
            for (int i = 0; i < LevelGiftDTs.Count; i++)
            {
                LevelGiftDT levelGiftDT = (LevelGiftDT)LevelGiftDTs[i];
                if(levelGiftDT.iId == StaticValue.OpenPopupLevelGiftId)
                {
                    f_UpdateInfo((LevelGiftDT)LevelGiftDTs[i], f_GetObject("Type1Panel"));
                    index_LevelGiftDTs = i;
                    StaticValue.OpenPopupLevelGiftId = 0;
                    UITool.CreatLocalDataLevelGift(levelGiftDT.iId, false);
                }
            }
        }
        else
        {
            f_UpdateInfo((LevelGiftDT)LevelGiftDTs[index_LevelGiftDTs], f_GetObject("Type1Panel"));
        }

        f_UpdateArrow();
        //UITool.f_OpenOrCloseWaitTip(true);
        //Data_Pool.m_EventTimePool.f_LevelGiftPool(eventTimeDT.iId, RequestGetCallback);
    }

    private int InitKey(int idA, int idB)
    {
        string key = idA + "" + idB;
        return int.Parse(key);
    }

    private void UpdateItem(GameObject go, ResourceCommonDT Data)
    {
        Transform tran = go.transform;

        UI2DSprite Icon = tran.Find("Icon").GetComponent<UI2DSprite>();
        UILabel Num = tran.Find("Num").GetComponent<UILabel>();
        UILabel Name = tran.Find("Name").GetComponent<UILabel>();
        UISprite Case = tran.Find("Case").GetComponent<UISprite>();

        Icon.sprite2D = UITool.f_GetIconSprite(Data.mIcon);
Num.text = string.Format("Number: {0}", Data.mResourceNum);
        Name.text = Data.mName.ToString();
        Case.spriteName = UITool.f_GetImporentCase(Data.mImportant);
        //CreareEffect(tran.Find("Effect").gameObject, Data.mImportant);

        f_RegClickEvent(Icon.gameObject, OnClickItem, Data);
    }

    private void CreareEffect(GameObject EquipObj, int Imporent)
    {
        string EffectName = "";
        switch ((EM_Important)Imporent)
        {
            case EM_Important.White:
            case EM_Important.Green:
                EffectName = UIEffectName.biankuangliuguang_green;
                break;
            case EM_Important.Blue:
                EffectName = UIEffectName.biankuangliuguang_bue;
                break;
            case EM_Important.Purple:
                EffectName = UIEffectName.biankuangliuguang_purple;
                break;
            case EM_Important.Oragen:
                EffectName = UIEffectName.biankuangliuguang_oragen;
                break;
            case EM_Important.Red:
                EffectName = UIEffectName.biankuangliuguang_red;
                break;
            case EM_Important.Gold:
                break;
        }

        for (int j = EquipObj.transform.childCount - 1; j >= 0; j--)//删除特效
            Destroy(EquipObj.transform.GetChild(j).gameObject);

        GameObject SetEquipEffect = UITool.f_CreateEffect_Old(EffectName, EquipObj.transform, Vector3.zero, 1f, 0, UIEffectName.UIEffectAddress1);
        SetEquipEffect.GetComponent<ParticleScaler>().TrailRenderSortingOrder = 1;
        SetEquipEffect.transform.parent.localScale = Vector3.one * 120;
        SetEquipEffect.transform.parent.localPosition = Vector3.zero;
        SetEquipEffect.transform.localPosition = Vector3.zero;
        SetEquipEffect.transform.localScale = Vector3.one;
    }
    private float updateTime = 0f;
    protected override void f_Update()
    {
        base.f_Update();
        updateTime += Time.deltaTime;
        if (updateTime > 1f)
        {
            updateTime = 0f;
            if(iTime1 > 0)
            {
                iTime1--;
                tDate1 = new System.DateTime(1970, 1, 1, 0, 0, 0, System.DateTimeKind.Unspecified).AddSeconds(iTime1);
                ActivityEndTime.text = string.Format(CommonTools.f_GetTransLanguage(2112), tDate1.Day - 1, tDate1.Hour, tDate1.Minute) + tDate1.Second + CommonTools.f_GetTransLanguage(2114);
            }
            else
            {
                ActivityEndTime.text = string.Format(CommonTools.f_GetTransLanguage(2115));
            }
        }
    }


    private void OnClickItem(GameObject go, object obj1, object obj2)
    {
        ResourceCommonDT commonData = (ResourceCommonDT)obj1;
        ccUIManage.GetInstance().f_SendMsg(UINameConst.ResourceCommonItemDetailPage, UIMessageDef.UI_OPEN, commonData);
    }

    //查询成功回调
    private void OnSucCallback(object obj)
    {
        ccUIHoldPool.GetInstance().f_Hold(this);
    }

    private void OnGetSucCallback(object value)
    {
        //server đã gởi về pool

        UITool.f_OpenOrCloseWaitTip(false);

    }

    private void OnGetFailCallback(object obj)
    {
        UITool.Ui_Trip(CommonTools.f_GetTransLanguage(1274));
    }

    public void f_DestoryView()
    {
        

    }

    public bool CheckDoneEvent(int eventTimeId)
    {

        return false;
    }

}


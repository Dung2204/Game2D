...
public class ChaosBattleFinishPage : UIFramwork
{
    ...
    {
        base.UI_OPEN(e);
        bool isWin = Data_Pool.m_ChaosBattlePool.BattleResult > 0;
        m_WinObj.SetActive(isWin);
        m_LoseObj.SetActive(!isWin);
m_WinScore.text = string.Format("Score: +{0}",Data_Pool.m_ChaosBattlePool.ResultScore);
m_LoseScore.text = string.Format("Score: +{0}", Data_Pool.m_ChaosBattlePool.ResultScore);
        ChaosBattlePoolDT enemy = Data_Pool.m_ChaosBattlePool.EnemyInfo;
    ...
    private void f_UpdateEnemyRole(GameObject parent, ChaosBattlePoolDT enemyInfo)
    {
    ...
    {
        ccUIManage.GetInstance().f_SendMsg(UINameConst.ChaosBattleFinishPage, UIMessageDef.UI_CLOSE);

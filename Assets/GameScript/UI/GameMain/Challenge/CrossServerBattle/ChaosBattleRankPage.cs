...
public class ChaosBattleRankPage : UIFramwork
{
    ...
    private UIWrapComponent m_RankListWrapComponent
    {
        get
        {
            ...
            {
                m_RankList = Data_Pool.m_ChaosBattlePool.RankDict[(byte)m_CurZoneId].RankList;
                MessageBox.ASSERT("TsuLog CHECK RANKLIST CHAOS 1007: " + Data_Pool.m_ChaosBattlePool.RankDict[(byte)m_CurZoneId].RankList.Count);
                _rankListWrapComponent = new UIWrapComponent(88, 1, 800, 10, mRankItemParent, mRankItem, m_RankList, f_RankListUpdateByInfo, null);
            }
    ...
    private ChaosBattleRankItem mSelfRankItem;
    ...
    protected override void InitGUI()
    {
        ...
        mSelfRankItem = f_GetObject("SelfRankItem").GetComponent<ChaosBattleRankItem>();
        mTabs = new ChaosBattleRankTab[Data_Pool.m_ChaosBattlePool.RankDict.Keys.Count];
        int tabIdx = 0;
        foreach (byte key in Data_Pool.m_ChaosBattlePool.RankDict.Keys)
        {
            if (m_DefaultZoneId == 0)
                m_DefaultZoneId = Data_Pool.m_ChaosBattlePool.RankDict[key].ZoneTemplate.iId;
            GameObject go = NGUITools.AddChild(mTabGrid.gameObject, mTabItem);
            go.SetActive(true);
            mTabs[tabIdx] = go.GetComponent<ChaosBattleRankTab>();
            mTabs[tabIdx].f_Init(Data_Pool.m_ChaosBattlePool.RankDict[key].ZoneTemplate);
            f_RegClickEvent(mTabs[tabIdx].m_NormalBg, f_OnZoneTabClick, Data_Pool.m_ChaosBattlePool.RankDict[key].ZoneTemplate.iId);
            tabIdx++;
        }
    ...
    private void f_OnZoneTabClick(GameObject go, object value1, object value2)
    {
        ...
        Data_Pool.m_ChaosBattlePool.f_ExecuteAfterRankList((byte)m_CurZoneId, m_FirstPage, ref m_NeedUpdate, f_Callback_RankList);
    }

    private void f_OnBtnCloseClick(GameObject go, object value1, object value2)
    {
        ccUIManage.GetInstance().f_SendMsg(UINameConst.ChaosBattleRankPage, UIMessageDef.UI_CLOSE);
    }
    ...
    {
        ChaosBattleRankItem tItem = tf.GetComponent<ChaosBattleRankItem>();
        tItem.f_UpdateByInfo(false, (ChaosBattleRankPoolDT)node);
    }

    private void f_UpdateSelfItem()
    {
        mSelfRankItem.f_UpdateByInfo(true, Data_Pool.m_ChaosBattlePool.SelfRankInfo);
    }
}

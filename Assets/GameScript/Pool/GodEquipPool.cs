...
public class GodEquipPool : BasePool
{
    public GodEquipPool()
        : base("GodEquipPool", true)
    {
    }
    ...
    {
        SC_GodEquipInfo tServerData = (SC_GodEquipInfo)Obj;
        GodEquipPoolDT tPoolDataDT = new GodEquipPoolDT();
        ...
        Data_Pool.m_AwardPool.f_AddAward(EM_ResourceType.GodEquip, tPoolDataDT.m_iTempleteId, 1);
        f_Save(tPoolDataDT);
    }
    ...
    protected override void f_Socket_UpdateData(SockBaseDT Obj)
    {
        ...
        SC_GodEquipInfo tServerData = (SC_GodEquipInfo)Obj;
        GodEquipPoolDT tPoolDataDT = (GodEquipPoolDT)f_GetForId(tServerData.id);
        if (tPoolDataDT == null)
        {
    ...
    {
        SC_GodEquipInfo tSC_ItemInit = new SC_GodEquipInfo();
        GameSocket.GetInstance().f_RegMessage_Int1((int)SocketCommand.SC_GodEquipInfo, tSC_ItemInit, Callback_SocketData_Update);
        stPoolDelData tstPoolDelData = new stPoolDelData();
        GameSocket.GetInstance().f_RegMessage((int)SocketCommand.SC_GodEquipRemove, tstPoolDelData, Callback_Del);
        SC_GodEquipIntensify tSC_EquipIntensify = new SC_GodEquipIntensify();
        GameSocket.GetInstance().f_RegMessage((int)SocketCommand.SC_GodEquipIntensify, tSC_EquipIntensify, InterSuc);
        //SC_EquipCostHistory tCS_EquipCostHistory = new SC_EquipCostHistory();
        //GameSocket.GetInstance().f_RegMessage((int)SocketCommand.SC_GodEquipCostHistory, tCS_EquipCostHistory, tEquipCostHistory);
    ...
    {
        SC_GodEquipIntensify tSC_EquipIntensify = (SC_GodEquipIntensify)obj;
    ...
    {
        ShowTirt = back;
        GameSocket.GetInstance().f_RegCommandReturn((int)SocketCommand.CS_GodEquipIntensifyOneKey, tSocketCallbackDT.m_ccCallbackSuc, tSocketCallbackDT.m_ccCallbackFail);
        CreateSocketBuf tCreateSocketBuf = new CreateSocketBuf();
        tCreateSocketBuf.f_Add(id);
        tCreateSocketBuf.f_Add(dstLv);
        byte[] bBuf = tCreateSocketBuf.f_GetBuf();
        GameSocket.GetInstance().f_SendBuf(SocketCommand.CS_GodEquipIntensifyOneKey, bBuf);
        MessageBox.DEBUG("Gửi" + (int)SocketCommand.CS_GodEquipIntensifyOneKey);
    }
    /// <summary>
    /// 装备升级
    /// </summary>
    ...
    {
        GameSocket.GetInstance().f_RegCommandReturn((int)SocketCommand.CS_GodEquipRefine, tSocketCallbackDT.m_ccCallbackSuc, tSocketCallbackDT.m_ccCallbackFail);
        CreateSocketBuf tCreateSocketBuf = new CreateSocketBuf();
        tCreateSocketBuf.f_Add(EquipId);
        tCreateSocketBuf.f_Add(tempId);
        tCreateSocketBuf.f_Add(num);
        byte[] bBuf = tCreateSocketBuf.f_GetBuf();
        GameSocket.GetInstance().f_SendBuf(SocketCommand.CS_GodEquipRefine, bBuf);
    }
    /// <summary>
    /// 装备升星
    /// </summary>
    ...
    public void f_GodEquipUpStar(long EquipId, EM_GodEquipUpStarType upType, SocketCallbackDT tSocketCallbackDT)
    {
        GameSocket.GetInstance().f_RegCommandReturn((int)SocketCommand.CS_GodEquipStarsup, tSocketCallbackDT.m_ccCallbackSuc, tSocketCallbackDT.m_ccCallbackFail);
        CreateSocketBuf tCreateSocketBuf = new CreateSocketBuf();
        tCreateSocketBuf.f_Add(EquipId);
        tCreateSocketBuf.f_Add((byte)upType);
        byte[] bBuf = tCreateSocketBuf.f_GetBuf();
        GameSocket.GetInstance().f_SendBuf(SocketCommand.CS_GodEquipStarsup, bBuf);
    }
    ...
    {
        int result = 0;
        foreach (GodEquipPoolDT item in f_GetAll())
        {
    ...
    /// 装备重生
    /// </summary>
    public void f_GodEquipRebirth(long dstId, SocketCallbackDT back)
    {
        GameSocket.GetInstance().f_RegCommandReturn((int)SocketCommand.CS_Rebirth, back.m_ccCallbackSuc, back.m_ccCallbackFail);
        CreateSocketBuf tCreateSocketBuf = new CreateSocketBuf();
        tCreateSocketBuf.f_Add((byte)EM_ResourceType.GodEquip);
    ...
    public void f_GodEquipOneKeyRefine(long id, int num, int[] RefineItemId, int[] RefineItemNum, SocketCallbackDT tSocketCallback)
    {
        GameSocket.GetInstance().f_RegCommandReturn((int)SocketCommand.CS_GodEquipRefineOnekey, tSocketCallback.m_ccCallbackSuc, tSocketCallback.m_ccCallbackFail);
        CreateSocketBuf tCreateSocketBuf = new CreateSocketBuf();
        tCreateSocketBuf.f_Add(id);
        tCreateSocketBuf.f_Add(num);
        ...
        byte[] bBuf = tCreateSocketBuf.f_GetBuf();
        GameSocket.GetInstance().f_SendBuf(SocketCommand.CS_GodEquipRefineOnekey, bBuf);
    ...
    public bool f_CheckGodEquipRefin(GodEquipPoolDT godEquip)
    {
        int[] Tempid = UITool.f_GetGoodsForEffect(EM_GoodsEffect.GodEquipRefineExp);
        for (int i = 0; i < Tempid.Length; i++)
        {
            if(UITool.f_GetGoodsNum(Tempid[i]) > 0)
            {
                return true;
            }
        }
    ...
    //public bool f_CheckGodEquipCanConsume(GodEquipPoolDT godEquip)
    //{
    //    if (godEquip.m_ExpIntensify == 0 && godEquip.m_lvIntensify == 1 && godEquip.m_lvRefine == 0)
    //    {
    //        return true;
    //    }

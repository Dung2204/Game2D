...
/// 跨服战(跨服演武) pool
/// </summary>
public class CrossServerBattlePool : BasePool
{
    /// <summary>
    ...
    private CrossServerBattleZoneDT m_CurZoneTemplate;


    /// 当前头衔Id 通过TitleId来设置 
    private int m_CurTitleId;
    //头衔模板 不需要手动设置 TitleId会自己设置
    private CrossServerBattleTitleDT m_CurTitleTemplate;
    //当前头衔星数
    ...
    //对手玩家信息
    private CrossServerBattlePoolDT m_EnemyInfo;
    ...
    public int ZoneId
    {
        ...
        {
            m_CurZoneId = value;
            m_CurZoneTemplate = (CrossServerBattleZoneDT)glo_Main.GetInstance().m_SC_Pool.m_CrossServerBattleZoneSC.f_GetSC(value);
            if (m_CurZoneTemplate == null)
            {
MessageBox.ASSERT("Model ID does not exist, id:" + value);
                //容错
                m_CurZoneTemplate = (CrossServerBattleZoneDT)glo_Main.GetInstance().m_SC_Pool.m_CrossServerBattleZoneSC.f_GetSC(1);
            }
        }
        get
        {
    ...
    /// 战区模板
    /// </summary>
    public CrossServerBattleZoneDT ZoneTemplate
    {
        get
        {
    ...
    public int TitleId
    {
        ...
        {
            m_CurTitleId = value;
            m_CurTitleTemplate = (CrossServerBattleTitleDT)glo_Main.GetInstance().m_SC_Pool.m_CrossServerBattleTitleSC.f_GetSC(value);
            if (m_CurZoneTemplate == null)
            {
MessageBox.ASSERT("Formal Title Setup ID does not exist,id:" + value);
                //容错
                m_CurTitleTemplate = (CrossServerBattleTitleDT)glo_Main.GetInstance().m_SC_Pool.m_CrossServerBattleTitleSC.f_GetSC(1);
            }
        }
        get
        {
    ...
    /// 官衔模板
    /// </summary>
    public CrossServerBattleTitleDT TitleTemplate
    {
        get
        {
    ...
    /// 对手玩家信息
    /// </summary>
    public CrossServerBattlePoolDT EnemyInfo
    {
        get
        {
            return m_EnemyInfo;
    ...
    public CrossServerBattlePool() : base("CrossServerPoolDT", true)
    {
        //根据参数表数据初始化
        GameParamDT tParam = glo_Main.GetInstance().m_SC_Pool.m_GameParamSC.f_GetSC((int)EM_NeedLevel.CrossServerBattle) as GameParamDT;
        if (tParam != null)
        {
    ...
    protected override void f_Init()
    {
        ...
        //官衔最大星数
        m_TitleStarMax = 0;
        List<NBaseSCDT> tList = glo_Main.GetInstance().m_SC_Pool.m_CrossServerBattleTitleSC.f_GetAll();
        for (int i = 0; i < tList.Count; i++)
        {
            CrossServerBattleTitleDT tNode = (CrossServerBattleTitleDT)tList[i];
            if (tNode.iStarNum > m_TitleStarMax)
                m_TitleStarMax = tNode.iStarNum;
        }
        m_EnemyInfo = new CrossServerBattlePoolDT();

        //初始排行榜数据
        m_RankDict = new Dictionary<byte, CSBattleRankZone>();
        List<NBaseSCDT> tZoneIniList = glo_Main.GetInstance().m_SC_Pool.m_CrossServerBattleZoneSC.f_GetAll();
        for (int i = 0; i < tZoneIniList.Count; i++)
        {
            CrossServerBattleZoneDT tNode = (CrossServerBattleZoneDT)tZoneIniList[i];
            if (!m_RankDict.ContainsKey((byte)tNode.iId))
                m_RankDict.Add((byte)tNode.iId, new CSBattleRankZone(tNode));
            else
MessageBox.ASSERT("Available with the same ID");
    ...
    {
        CMsg_SC_CrossBattleInfor tSC_CrossBattleInfor = new CMsg_SC_CrossBattleInfor();
        GameSocket.GetInstance().f_RegMessage((int)SocketCommand.SC_CrossBattleInfor, tSC_CrossBattleInfor, f_SC_CrossBattleInfoHandle);

        CMsg_SC_CrossBattlePlayer tSC_CrossBattlePlayer = new CMsg_SC_CrossBattlePlayer();
        GameSocket.GetInstance().f_RegMessage((int)SocketCommand.SC_CrossBattlePlayer, tSC_CrossBattlePlayer, f_SC_CrossBattlePlayerHandle);

        CMsg_SC_CrossBattleResult tSC_CrossBattleResult = new CMsg_SC_CrossBattleResult();
        GameSocket.GetInstance().f_RegMessage((int)SocketCommand.SC_CrossBattleResult, tSC_CrossBattleResult, f_SC_CrossBattleResultHandle);

        CMsg_SC_CrossRankNode tSC_CrossRankNode = new CMsg_SC_CrossRankNode();
        GameSocket.GetInstance().f_RegMessage_Int1((int)SocketCommand.SC_CrossRank, tSC_CrossRankNode, f_SC_CrossRankHandle);

        CMsg_SC_CrossRankNode tSC_CrossRankNodeSelf = new CMsg_SC_CrossRankNode();
        GameSocket.GetInstance().f_RegMessage((int)SocketCommand.SC_CrossRankSelf, tSC_CrossRankNodeSelf, f_SC_CrossRankSelfHandle);
    }

    #region 协议处理

    private void f_SC_CrossBattleInfoHandle(object value)
    {
        CMsg_SC_CrossBattleInfor serverInfo = (CMsg_SC_CrossBattleInfor)value;
        m_IsInitInfo = true;
        m_WinScore = 0;
        m_LoseScore = 0;
        ...
        glo_Main.GetInstance().m_UIMessagePool.f_Broadcast(UIMessageDef.UI_CrossServerBattle_TimesUpdate, m_BattleBuyTimes);
    }

    private void f_SC_CrossBattlePlayerHandle(object value)
    {
        CMsg_SC_CrossBattlePlayer serverInfo = (CMsg_SC_CrossBattlePlayer)value;
        //m_EnemyInfo.f_UpdateInfo(serverInfo.userId, serverInfo.iServerId, serverInfo.iTempId, 
        //    serverInfo.uFrameId, serverInfo.power, serverInfo.szRoleName);

        //TsuCode - Add Server id
        m_EnemyInfo.f_UpdateInfo(serverInfo.userId, serverInfo.iCrossSvId, serverInfo.iTempId,
    ...
    {
        CMsg_SC_CrossBattleResult serverInfo = (CMsg_SC_CrossBattleResult)value;
        m_IsFinish = true;
            ...
            m_MatchSocketCallbackDt.m_ccCallbackFail != null)
        {
            ...
            {
                //更新战斗数据
                StaticValue.m_CurBattleConfig.f_UpdateInfo(EM_Fight_Enum.eFight_CrossServerBattle, 0, 0, 0);
    ...
    private void f_SC_CrossRankHandle(int zoneId, int value2, int num, System.Collections.ArrayList arrayList)
    {
...
MessageBox.ASSERT(string.Format("The server sent data from regions that do not exist，zoneId:{0}", zoneId));
        ...
        for (int i = 0; i < arrayList.Count; i++)
        {
            ...
            CrossServerBattleRankPoolDT tNode = new CrossServerBattleRankPoolDT(serverData.uRankIdx, serverData.uRankTitle, serverData.uServerId, serverData.uPower, serverData.szRoleName);
            m_RankDict[(byte)zoneId].f_AddRankNode(tNode);
        }
    }

    private void f_SC_CrossRankSelfHandle(object value)
    {
        CMsg_SC_CrossRankNode serverData = (CMsg_SC_CrossRankNode)value;
        m_SelfRankInfo = new CrossServerBattleRankPoolDT(serverData.uRankIdx,serverData.uRankTitle,serverData.uServerId,serverData.uPower,serverData.szRoleName);
    }
    ...
    public void f_CrossServerBattleInit(SocketCallbackDT socketCallbackDt)
    {
        ...
        GameSocket.GetInstance().f_RegCommandReturn((int)SocketCommand.CS_CrossBattleInfor, socketCallbackDt.m_ccCallbackSuc, socketCallbackDt.m_ccCallbackFail);
        CreateSocketBuf tCreateSocketBuf = new CreateSocketBuf();
        byte[] bBuf = tCreateSocketBuf.f_GetBuf();
        GameSocket.GetInstance().f_SendBuf(SocketCommand.CS_CrossBattleInfor, bBuf);
    }
    ...
    public void f_MatchBattle(SocketCallbackDT socketCallbackDt)
    {
        ...
        GameSocket.GetInstance().f_RegCommandReturn((int)SocketCommand.CS_CrossBattle, socketCallbackDt.m_ccCallbackSuc, socketCallbackDt.m_ccCallbackFail);
        CreateSocketBuf tCreateSocketBuf = new CreateSocketBuf();
        byte[] bBuf = tCreateSocketBuf.f_GetBuf();
        GameSocket.GetInstance().f_SendBuf(SocketCommand.CS_CrossBattle, bBuf);
    }
    ...
    {
        GameSocket.GetInstance().f_RegCommandReturn((int)SocketCommand.CS_CrossBattleBuyTims, socketCallbackDt.m_ccCallbackSuc, socketCallbackDt.m_ccCallbackFail);
        CreateSocketBuf tCreateSocketBuf = new CreateSocketBuf();
        tCreateSocketBuf.f_Add(buyTimes);
        byte[] bBuf = tCreateSocketBuf.f_GetBuf();
        GameSocket.GetInstance().f_SendBuf(SocketCommand.CS_CrossBattleBuyTims, bBuf);
    }

    public void f_RankList(short zoneId, byte pageIdx, SocketCallbackDT socketCallbackDt)
    {
        GameSocket.GetInstance().f_RegCommandReturn((int)SocketCommand.CS_CrossRank, socketCallbackDt.m_ccCallbackSuc, socketCallbackDt.m_ccCallbackFail);
        CreateSocketBuf tCreateSocketBuf = new CreateSocketBuf();
        tCreateSocketBuf.f_Add(zoneId);
        tCreateSocketBuf.f_Add(pageIdx);
        byte[] bBuf = tCreateSocketBuf.f_GetBuf();
        GameSocket.GetInstance().f_SendBuf(SocketCommand.CS_CrossRank, bBuf);
    }

    public void f_RankListSelf(SocketCallbackDT socketCallbackDt)
    {
        GameSocket.GetInstance().f_RegCommandReturn((int)SocketCommand.CS_CrossRankSelf, socketCallbackDt.m_ccCallbackSuc, socketCallbackDt.m_ccCallbackFail);
        CreateSocketBuf tCreateSocketBuf = new CreateSocketBuf();
        byte[] bBuf = tCreateSocketBuf.f_GetBuf();
        GameSocket.GetInstance().f_SendBuf(SocketCommand.CS_CrossRankSelf, bBuf);
    }
    ...
    private Dictionary<byte, CSBattleRankZone> m_RankDict;

    private int m_SelfRequestTime = 0;
    private CrossServerBattleRankPoolDT m_SelfRankInfo;

    /// <summary>
    /// 排行榜数据
    /// </summary>
    public Dictionary<byte, CSBattleRankZone> RankDict
    {
        get
        {
            return m_RankDict;
    ...
    /// 个人排行榜信息
    /// </summary>
    public CrossServerBattleRankPoolDT SelfRankInfo
    {
        get
        {
...
public class CSBattleRankZone
{
    ...
    public CSBattleRankZone(CrossServerBattleZoneDT zoneTemplate)
    {
    ...
    public void f_AddRankNode(CrossServerBattleRankPoolDT rankNode)
    {

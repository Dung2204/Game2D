...
/// 跨服战商店
/// </summary>
public class ChaosBattleShopPool : BasePool
{
    ...
    public ChaosBattleShopPool() : base("ChaosBattleShopPoolDT")
    {
    ...
    protected override void f_Init()
    {
        ...
        List<NBaseSCDT> tInitList = glo_Main.GetInstance().m_SC_Pool.m_ChaosBattleShopSC.f_GetAll();
        for (int i = 0; i < tInitList.Count; i++)
        {
            ChaosBattleShopDT tInitNode = (ChaosBattleShopDT)tInitList[i];
            f_Save(new ChaosBattleShopPoolDT(tInitNode));
        }
        f_SortData1();
    ...
    private void f_UpdateDataByLv(object value)
    {
        ...
        {
            ChaosBattleShopPoolDT tNode = (ChaosBattleShopPoolDT)tSourceList[i];
    ...
    private void f_UpdateDataByNextDay(object value)
    {
        ...
        {
            ChaosBattleShopPoolDT tNode = (ChaosBattleShopPoolDT)tResetList[i];
            tNode.f_Reset();
        }
    }
    ...
    protected override void RegSocketMessage()
    {
        ...
        GameSocket.GetInstance().f_RegMessage_Int1((int)SocketCommand.SC_ShopChaosInfo, tSC_GoodNode, f_SC_ShopInfoUpdateHandle);
    }
    ...
    public void f_SC_ShopInfoUpdateHandle(int value1, int value2, int num, System.Collections.ArrayList arrayList)
    {
        for (int i = 0; i < arrayList.Count; i++)
        {
            ...
            ChaosBattleShopPoolDT shopItem = (ChaosBattleShopPoolDT)f_GetForId(serverData.goodsId);
            if (shopItem != null)
            {
...
MessageBox.ASSERT(string.Format("The server sent item data does not exist,NodeType:{0}; GoodId:{1}", value1, serverData.goodsId));
    ...
    public void f_ShopInit(SocketCallbackDT socketCallbackDt)
    {
        ...
        m_IsShopInit = true;
        GameSocket.GetInstance().f_RegCommandReturn((int)SocketCommand.CS_ShopChaosInfo, socketCallbackDt.m_ccCallbackSuc, socketCallbackDt.m_ccCallbackFail);
        CreateSocketBuf tCreateSocketBuf = new CreateSocketBuf();
        byte[] bBuf = tCreateSocketBuf.f_GetBuf();
        GameSocket.GetInstance().f_SendBuf(SocketCommand.CS_ShopChaosInfo, bBuf);
    }

    public void f_Buy(int shopItemId,int buyNum, SocketCallbackDT socketCallbackDt)
    {
        GameSocket.GetInstance().f_RegCommandReturn((int)SocketCommand.CS_ShopChaosBuy, socketCallbackDt.m_ccCallbackSuc, socketCallbackDt.m_ccCallbackFail);
        CreateSocketBuf tCreateSocketBuf = new CreateSocketBuf();
        tCreateSocketBuf.f_Add(shopItemId);
        tCreateSocketBuf.f_Add(buyNum);
        byte[] bBuf = tCreateSocketBuf.f_GetBuf();
        GameSocket.GetInstance().f_SendBuf(SocketCommand.CS_ShopChaosBuy, bBuf);
    }

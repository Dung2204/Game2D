...
public class TripleMoneySC : NBaseSC {
	public TripleMoneySC() {
		Create("TripleMoneyDT", true);
	}
	...
	private void DispSaveData(string ppSQL) {
		...
		for (int i = 0; i < tFoddScData.Length; i++) {
			try {
				...
				DataDT.iFirstPayNum = ccMath.atoi(tData[a++]);
				DataDT.iSecondPayNum = ccMath.atoi(tData[a++]);
				DataDT.iThirdPayNum = ccMath.atoi(tData[a++]);
	...
	{
		return f_GetAll().Where(o => o is TripleMoneyDT data && data.iEventTime == iId).ToList();
	}
}

...
public class BattlePassAwardSC : NBaseSC {
	public BattlePassAwardSC() {
		Create("BattlePassAwardDT", true);
	}
	...
	private void DispSaveData(string ppSQL) {
		...
		for (int i = 0; i < tFoddScData.Length; i++) {
			try {
				...
				int a = 0;
				DataDT = new BattlePassAwardDT();
				DataDT.iId = ccMath.atoi(tData[a++]);
				if (DataDT.iId <= 0) {
					MessageBox.ASSERT("Id错误");
				}
				DataDT.iLevel = ccMath.atoi(tData[a++]);
				DataDT.iScore = ccMath.atoi(tData[a++]);
				DataDT.szAwardLow = tData[a++];
				DataDT.szAwardHight = tData[a++];

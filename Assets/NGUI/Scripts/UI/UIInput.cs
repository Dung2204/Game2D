...
public class UIInput : MonoBehaviour
{
	...
	/// </summary>
	string VK = "àáãạảăắằẳẵặâấầẩẫậèéẹẻẽêềếểễệđìíĩỉịòóõọỏôốồổỗộơớờởỡợùúũụủưứừửữựỳỵỷỹýÀÁÃẠẢĂẮẰẲẴẶÂẤẦẨẪẬÈÉẸẺẼÊỀẾỂỄỆĐÌÍĨỈỊÒÓÕỌỎÔỐỒỔỖỘƠỚỜỞỠỢÙÚŨỤỦƯỨỪỬỮỰỲỴỶỸÝ";
	protected char Validate (string text, int pos, char ch)
	{
		...
		else if (validation == Validation.NameVN)
		{
			char[] strVKArray = VK.ToCharArray();
			if (ch >= 'A' && ch <= 'Z') return ch;
			if (ch >= 'a' && ch <= 'z') return ch;
			if (ch >= '0' && ch <= '9') return ch;
			else
            {
                for (int i = 0; i < strVKArray.Length; i++)
                {
					if (ch == strVKArray[i]) return ch;
                }	
            }
		}
		return (char)0;
